...
condition_summary <- bridge_data %>%
  summarise(
    ...
    "Standard Deviation" = sd(Condition, na.rm = TRUE),
    Count = n()
  )
...
```
@tbl-condition-summary presents for the condition ratings of bridges in New York State, based on a dataset of 15,855 observations. The mean condition rating of 5.38 indicates that, on average, bridges are in fair to good condition. The median rating of 5.30 suggests that half of the bridges have condition ratings below or equal to 5.30. The close alignment of the mean and median reflects a relatively symmetrical distribution. While the minimum condition rating is 1.93, and the maximum is 7, the standard deviation of 0.83 suggests that most bridges cluster around the mean, with only a few outliers at the extreme ends of the scale. These statistics indicate that the majority of bridges are maintained at a stable and adequate level, with relatively few in very poor or excellent condition.
...
#| tbl-cap: "Proportion of Bridges by Municipality. The first column shows the type of municipality (Located_Municipality), and the second column represents the proportion of bridges in each category (Percentage Account)."
#| echo: false
#| tbl-pos: "H"
...
#| tbl-cap: "Average Bridge Condition by Municipality. The first column shows the type of municipality (Located_Municipality), and the second column provides the average condition rating (Average Condition) for bridges in each category."
#| echo: false
#| tbl-pos: "H"
...
located_municipality_condition <- bridge_data %>%
  group_by(Located_Municipality) %>%
  summarise(
    "Average Condition" = round(mean(Condition, na.rm = TRUE), 2)
  )
...
#| tbl-cap: "Proportion of Bridges by Ownership. The first column shows the type of ownership group (Owner_Group), and the second column represents the proportion of bridges in each category (Percentage Account)."
#| echo: false
#| tbl-pos: "H"
...
#| tbl-cap: "Average Bridge Condition by Ownership. The first column shows the type of ownership group (Owner_Group), and the second column provides the average condition rating (Average Condition) for bridges in each category."
#| echo: false
#| tbl-pos: "H"
